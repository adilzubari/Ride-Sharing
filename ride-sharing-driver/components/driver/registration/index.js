...
export default function index(props) {
  ...
  const pickIdentityCard = async () => {
    ...
    if (!result.cancelled) setIdentityCard(result.base64);
  };
  ...
  const pickCarDocument = async () => {
    ...
    if (!result.cancelled) setCarDocuments(result.base64);
  };
  ...
  const pickDrivingLicense = async () => {
    ...
    if (!result.cancelled) setDrivingLicense(result.base64);
  };
  ...
  if (RequestPending) {
    return (
      ...
      >
        <Title style={{ color: colors.primary.lighter }}>Request Created</Title>
        <Subheading>Request is in review</Subheading>
        <Paragraph
          style={{
        ...
        >
          You willn't be able to login untill you are approved by MauCarPool
          Team
        </Paragraph>
        <Paragraph>Patience is the key ☺️</Paragraph>
      </View>
    );
  } else
    return (
      <ScrollView style={styles.body}>
        ...
        <View style={styles.loginForm}>
          ...
          >
            Please note that you'll not be able to use the application untill
            your documents are verified, so we advice to upload the legal
            documents
          </Text>
        </View>
